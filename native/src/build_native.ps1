...
Run-Cl ($common + @('/O2', $bridgeCpp, '/link', ('/OUT:' + $bridgeDll), 'd3d12.lib', 'dxgi.lib', 'ole32.lib') + ($libDirs | ForEach-Object { '/LIBPATH:' + $_ })) '[2/3] Building in-process ComfyUI bridge...'

$hostCpp = Join-Path $Native 'dlss5nr_host.cpp'
$hostExe = Join-Path $Bin 'dlss5nr_host.exe'
$hostCommon = @('/nologo','/std:c++17','/EHsc','/O2','/MT')
foreach ($inc in $includeDirs) { $hostCommon += ('/I' + $inc) }
...
Write-Host 'For Linux/Wine video upscale (>1x), also place the matching nvngx_dlss.dll in:'
Write-Host "  $(Join-Path $Root 'runtime\nvngx_dlss.dll')"
